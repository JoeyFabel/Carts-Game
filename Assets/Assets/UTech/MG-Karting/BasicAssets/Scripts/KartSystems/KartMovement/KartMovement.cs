...
namespace KartGame.KartSystems
{
    ...
    public class KartMovement : MonoBehaviour, IKartCollider, IMovable, IKartInfo
    {
       ...
       public void ChangeInputToGamepad() //Added so that you can change control mode
        {
            m_Input = gameObject.GetComponent<GamepadInput>();
            print("input mode changed to gamepad");
        }

        public void ChangeInputToKeyboard() //Added so that you can change control mode
        {
            m_Input = gameObject.GetComponent<KeyboardInput>();
            print("input mode changed to keyboard");
        }
        ...
        void FixedUpdate ()
        {
            ...
            //starts sparks for each stage of drift
          //  if (driftTime >= GameControl.control.minimumDriftTime && driftTime < GameControl.control.bigDriftTime)
         //   {
         //       gameObject.GetComponent<DriftBoost>().littleBoostSparks();
          //  } else if (driftTime >= GameControl.control.bigDriftTime)
         //   {
         //       gameObject.GetComponent<DriftBoost>().bigBoostSparks();
          //  }
        ...
        void StopDrift (float deltaTime)
        {
            if (!m_Input.HopHeld || !m_HasControl)
            {
                ...
                if (isDrifting)
                {
                    gameObject.GetComponent<DriftBoost>().DriftOver(driftTime);
                }
